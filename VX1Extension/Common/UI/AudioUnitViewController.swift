...
private let log = Logger(subsystem: "com.taylor.audio.VX1Extension", category: "AudioUnitViewController")

@MainActor
public class AudioUnitViewController: AUViewController, AUAudioUnitFactory {
    var audioUnit: AUAudioUnit?
    
    var hostingController: HostingController<VX1ExtensionMainView>?
	...
	*/
	...
	nonisolated public func createAudioUnit(with componentDescription: AudioComponentDescription) throws -> AUAudioUnit {
		return try DispatchQueue.main.sync {
			...
			audioUnit = try VX1ExtensionAudioUnit(componentDescription: componentDescription, options: [])
			
			guard let audioUnit = self.audioUnit as? VX1ExtensionAudioUnit else {
				log.error("Unable to create VX1ExtensionAudioUnit")
				return audioUnit!
			}
			...
			audioUnit.setupParameterTree(VX1ExtensionParameterSpecs.createAUParameterTree())
